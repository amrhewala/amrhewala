  ...
  return (
    <div className="grid gap-6">
      {categories.map((category) => (
        <Card 
          key={category} 
          className="bg-white/5 backdrop-blur-sm border border-accent/10 transform transition-all duration-500 hover:shadow-lg"
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {skills
                ...
                .map((skill, index) => (
                  <div 
                    key={index} 
                    className={`rounded-lg p-2.5 transition-all duration-300 ${getSkillBadgeColor(skill.level)}`}
                  >
