...
#SuezFreeZone #FDIIntelligence #GlobalFreeZones #FinancialTimes #EconomicGrowth #DataAnalysis #Leadership`,
  ...
  photos: ["/Suez_free_zone-min.png", "/fDi-FZotY-2023-min.png", "/CONGRATULATIONS-min.png", "/2023-2.png"],
...
export const Achievements = () => {
  return <section className="relative bg-[#F1F0FB] py-0">
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {achievements.map((achievement, index) => <Card key={index} className="relative overflow-hidden hover:shadow-xl transition-all duration-300 border-[#D3E4FD]">
              ...
              <CardContent>
                ...
                {achievement.photos && <div className="mt-4 grid grid-cols-2 gap-4">
                    {achievement.photos.map((photo, photoIndex) => <Dialog key={photoIndex}>
                        <DialogTrigger>
                          <img src={photo} alt={`Achievement photo ${photoIndex + 1}`} className="rounded-lg shadow-md w-full h-auto cursor-pointer hover:opacity-80 transition-opacity" />
                        </DialogTrigger>
                        <DialogContent className="max-w-4xl">
                          <img src={photo} alt={`Achievement photo ${photoIndex + 1}`} className="w-full h-auto object-contain" />
                        </DialogContent>
                      </Dialog>)}
                  </div>}
                {achievement.link && <a href={achievement.link} target="_blank" rel="noopener noreferrer" className="text-[#0EA5E9] hover:text-[#F97316] underline inline-block mt-4">
